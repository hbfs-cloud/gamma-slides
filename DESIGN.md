...
components:
  action-orbit-hub:
    backgroundColor: "var(--gamma-text)"
    textColor: "var(--gamma-bg)"
    rounded: "50%"
    width: "56px"
    height: "56px"
  action-orbit-panel:
    backgroundColor: "var(--gamma-bg)"
    textColor: "var(--gamma-text)"
...
- A shared round M action menu with native controls, visible keyboard focus, reduced-motion fallback, and Appearance choices with shareable query values.
...
The flagship keeps the 1280×720 desktop stage between a fixed masthead and bottom navigation (56px each). Its ordinary sections use 54px horizontal gutters, 36px top padding, and 72px bottom clearance. Default story charts put the narrative above a wide plot; authored evidence charts instead place a 330px narrative column beside the plot with a 54px gap. The company remains in the masthead; Appearance and Studio open from the shared M menu, while chapter name, slide count, previous/next controls, and progress stay in the bottom rail.
...
The shared M hub is a 56px circle; its five branches are 48px circles on a 288px circular field. The Explorer panel uses 16px corners, with 4px native button/select corners. Other operational controls use 9px corners, 13–16px for floating tool surfaces, 20px for the setup dialog, and pills only for compact semantic status. Theme previews are framed compositions, not icon tiles.

## Components

### Shared M Action Menu
...
The standalone switcher is hidden when the shared M menu is active; Appearance reopens the same selector. Its retained fallback styling is:
...
- **Scenario identity:** Slide 33’s parallel coordinates expose the five unnamed source rows as stable Scenario A–E trajectories, each using a resolved theme series color. The scene legend remains visible as static evidence; native buttons in M → Explorer provide 44px targets, visible focus, and `aria-pressed` state. The legend’s outlined inactive swatch makes state visible without color alone. Enter and Space in the panel toggle trajectories without advancing the deck. All five axes retain source-derived bounds of 0–5 when a trajectory is hidden. Selection persists when charts rebuild for Appearance changes or responsive layout.
...
- **Entry:** `layout: chart`, `variant: immersive`, with supported bar or XYZ scatter data. A large open plot sits beside a comparison rail, with its view and selection commands in M → Explorer. Related bar tops are linked across series; scatter marks project onto their measured floor positions. The enclosing wire cube is absent.
...
- **Structure:** Fixed masthead and shared M menu for Appearance and Studio; fixed bottom navigation with previous/next, current chapter, slide count, and progress. The chapter index is an anchored, scrollable surface with six named destinations.
...
### Archify Diagram and Reading Controls

- **Scope:** `layout: diagram` accepts architecture, workflow, sequence, dataflow, and lifecycle specifications. The pinned Archify delivery pipeline produces the real SVG viewer. The adapter isolates its HTML and scripts inside a sandboxed iframe, frames the populated graph, and adjusts architecture/sequence text without editing vendor files or changing graph routes.
- **Control:** M → Explorer contains native play/pause, story and component selectors, overview reset, zoom, and “Lire les relations”. The panel mirrors the original controls and their state; the selected component’s name and outgoing relationships remain in the scene’s polite live readout. Complete relationship text opens in the panel. The source control strip and disclosure are hidden once the menu is ready.
- **Fullscreen:** M → Plein écran moves the active diagram into a viewport-sized native dialog and requests browser fullscreen. The dialog remains available if browser fullscreen is unsupported or rejected. A supported `moveBefore` preserves the live iframe runtime; other browsers remount and restore selected node/story and playback state. Return or Escape restores the diagram to its slide and focus to M. The iframe uses an opaque sandbox (`allow-scripts allow-downloads`, without `allow-same-origin`).
- **Lifecycle and export:** Only the active slide mounts its viewer. Slide exit, document hiding, print, and page exit remove it. Print and export retain the framed static SVG with interaction controls hidden. Theme changes update the embedded viewer’s theme and neutral field/text colors through the adapter.

**The Diagram Reading Rule.** Keep native component selection and complete text relationships reachable through M → Explorer, with relationship readouts beside the genuine SVG viewer. Frame populated content for the slide and selected content for narrow screens; preserve node identity, relationship labels, and routes through those framing changes.

### Optional Presenter Motion

`meta.motion: presenter` opts into finite, composition-aware entrances. Headers rise 14px over 520ms; up to eight rows rise 12px over 480ms, starting at 90ms with 65ms stagger. Diagram or story-chart fields reveal horizontally over 650ms after 100ms. All use `cubic-bezier(.2,.7,.2,1)`. Content remains visible without JavaScript. Slide changes cancel preceding animations; document hiding, print, page exit, and a reduced-motion preference cancel motion. Reduced motion and export skip entrances. Guided Archify playback remains an explicit local control, separate from the presenter entrance.
...
- **Interaction and access:** Pointer or touch selects a ribbon. The scene’s segment legend keeps complete names and both period values as static evidence. Five native buttons in M → Explorer expose the same information and `aria-pressed` state; arrows, Home, and End move focus, Enter/Space activate, and Escape clears selection. A live caption reports added revenue and the revenue multiple. Canvas content is hidden from assistive technology; the labeled SVG and HTML controls carry the evidence.
...
- **Reading and selection:** Wide chart fields sit between an Archivo heading and a caption rail; commands open through M → Explorer. Pointer, touch, or arrow-key selection reveals a live readout; selecting a network node emphasizes its connections. Text remains fully opaque when other marks dim.
...
- **Controls and values:** Drag and arrow keys directly change the view; 44px rotate controls and Reset are available in M → Explorer. Tapping a point has a 44px proximity target. The native “Inspect a company” selector in Explorer offers all five complete names and drives the same selection; a local live readout displays growth, EV/revenue, and margin with units. Escape on the plot clears selection. “Compare in 2D” and “Explore in 3D” keep the view reversible, and “View data” retains the exact-source dialog.
