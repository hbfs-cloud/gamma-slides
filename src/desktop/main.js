...
function bridgeRenderer(html) {
  const script = `<script>(()=>{const ready=async()=>{for(let i=0;i<80&&!window.Reveal;i+=1)await new Promise(r=>setTimeout(r,25));if(document.fonts?.ready)await document.fonts.ready.catch(()=>{});window.__GAMMA_PRESENTER_READY__=Boolean(window.Reveal);window.parent?.postMessage({type:'gamma-presenter-ready'},'*')};window.addEventListener('message',e=>{if(e.data?.type==='gamma-presenter-navigate'&&window.Reveal)window.Reveal.slide(Math.max(0,Number(e.data.index)||0))});ready()})()</script>`;
