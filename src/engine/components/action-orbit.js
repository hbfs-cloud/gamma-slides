...
export function actionOrbitCSS() { return `
  .gamma-orbit { position:fixed; right:28px; bottom:80px; z-index:1400; width:56px; height:56px; font:500 14px/1.4 Archivo,system-ui,sans-serif; color:var(--gamma-text); }
  .gamma-orbit button,.gamma-orbit select { font:inherit; color:inherit; cursor:pointer; }
  .gamma-orbit button:focus-visible,.gamma-orbit select:focus-visible { outline:2px solid var(--gamma-primary); outline-offset:4px; }
  .gamma-orbit-hub { position:relative; z-index:2; width:56px; height:56px; border-radius:50%; border:1px solid var(--gamma-text); background:var(--gamma-text); color:var(--gamma-bg)!important; font:650 20px/1 Archivo,sans-serif!important; }
  .gamma-orbit-hub[aria-expanded=true] { background:var(--gamma-primary); border-color:var(--gamma-primary); }
  .gamma-orbit-canopy { position:absolute; right:-12px; bottom:-32px; width:288px; height:288px; border-radius:50%; background:var(--gamma-bg); border:1px solid var(--gamma-muted); opacity:0; visibility:hidden; transform:scale(.25); transform-origin:248px 228px; transition:transform 220ms cubic-bezier(.16,1,.3,1),opacity 140ms,visibility 220ms; pointer-events:none; }
  ...
  .gamma-orbit-branch { position:absolute; left:4px; top:4px; width:48px; height:48px; padding:12px; border:1px solid var(--gamma-muted); border-radius:50%; background:var(--gamma-bg); opacity:0; visibility:hidden; transform:translate(0,0) scale(.5); transition:transform 240ms cubic-bezier(.16,1,.3,1),opacity 140ms,visibility 240ms; pointer-events:none; }
  .gamma-orbit[data-open=true] .gamma-orbit-branch { opacity:1; visibility:visible; transform:translate(var(--orbit-x),var(--orbit-y)) scale(1); pointer-events:auto; }
  ...
  .gamma-orbit-panel { position:fixed; right:28px; bottom:152px; width:340px; max-width:calc(100vw - 32px); max-height:calc(100dvh - 220px); overflow:auto; overscroll-behavior:contain; background:var(--gamma-bg); border:1px solid var(--gamma-muted); border-radius:16px; padding:20px; box-sizing:border-box; }
  ...
  @media(max-width:900px) { .gamma-orbit { right:20px; bottom:116px; }.gamma-orbit-panel { right:16px; bottom:152px; } }
  @media(prefers-reduced-motion:reduce) { .gamma-orbit-canopy,.gamma-orbit-branch { transition:opacity 100ms; } }
...
function initActionOrbit(icons) {
  if (new URLSearchParams(location.search).has('gamma-export') || new URLSearchParams(location.search).has('print-pdf')) return;
  ...
  const sourceSelector='.revenue-sculpture-key,.archify-controls,.archify-reading>label,.archify-data,.d3-webgpu-actions,.d3-depth-toolbar,.spatial-toolbar,.spatial-inspector>label,.cinema-actions,.cinema-tools,.chart-scenario-legend';
  function collect(){
  ...
  hub.addEventListener('pointerenter',event=>{if(event.pointerType==='mouse'&&matchMedia('(hover:hover)').matches&&panel.hidden){openedByHover=root.dataset.open!=='true';setOpen(true);}});
  ...
  hub.addEventListener('click',()=>{if(!panel.hidden){panel.hidden=true;setOpen(true);}else if(openedByHover){openedByHover=false;setOpen(true);}else setOpen(root.dataset.open!=='true');});
  root.querySelector('[data-orbit-close]').addEventListener('click',()=>{panel.hidden=true;setOpen(true);hub.focus();});
  root.addEventListener('keydown',event=>{event.stopPropagation();if(event.key==='Escape'){event.preventDefault();if(root.dataset.open!=='true'&&panel.hidden&&document.querySelector('.archify-fullscreen[open]'))document.querySelector('.archify-fullscreen[open] [data-archify-action=close]').click();else{setOpen(false);hub.focus();}}else if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(event.key)&&panel.hidden){event.preventDefault();setOpen(true);const buttons=[hub,...root.querySelectorAll('.gamma-orbit-branch:not(:disabled)')],direction=['ArrowUp','ArrowRight'].includes(event.key)?1:-1;buttons[(buttons.indexOf(document.activeElement)+direction+buttons.length)%buttons.length].focus();}});
  ...
  root.querySelectorAll('[data-orbit]').forEach(button=>button.addEventListener('click',()=>{
    ...
    else if(action==='studio'){initPresenterStudio();document.querySelector('[data-testid=studio-setup]')?.click();}
