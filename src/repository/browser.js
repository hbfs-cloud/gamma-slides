...
export function browserHandler({enabled=false,launch=launchBrowser}={}) {
 ...
 const getPage=async()=>{
  ...
  if(!starting)starting=(async()=>{
   try{
    browser=await launch({headless:true,defaultViewport:null,ignoreDefaultArgs:['--mute-audio'],args:[`--auto-select-tab-capture-source-by-title=${captureTitle}`,'--autoplay-policy=no-user-gesture-required','--disable-background-timer-throttling','--disable-renderer-backgrounding']});
    ...
    const old=browser;browser=page=controller=null;await old?.close().catch(()=>{});
    launchFailure=Error(`Isolated browser unavailable: ${error.message} It will not be retried until you explicitly close or restart this session.`);
    throw launchFailure;
   }
 ...
 const answer=async(origin,offer)=>{
  ...
  if(!controller||controller.isClosed()){
   ...
   let captureTimer;try{await Promise.race([controller.evaluate(async()=>{window.capture=await navigator.mediaDevices.getDisplayMedia({video:{width:{ideal:1920},height:{ideal:1080},frameRate:{ideal:30,max:30}},audio:{suppressLocalAudioPlayback:true,echoCancellation:false,noiseSuppression:false,autoGainControl:false},systemAudio:'exclude'});}),new Promise((_,reject)=>{captureTimer=setTimeout(()=>reject(Error('Browser capture timed out. Close and reopen the browser.')),15000);})]);}
  ...
  return controller.evaluate(async({description,epoch})=>{
   ...
   if(peer.iceGatheringState!=='complete')await new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(Error('WebRTC signaling timed out')),10000);peer.addEventListener('icegatheringstatechange',()=>{if(peer.iceGatheringState==='complete'){clearTimeout(timer);resolve();}});});
 ...
 const handler=async(req,res,path)=>{
  ...
  if(!enabled||req.headers.host!==new URL(origin).host||!['127.0.0.1','::ffff:127.0.0.1'].includes(req.socket.remoteAddress)||req.headers['sec-fetch-site']==='cross-site'||req.headers.origin&&req.headers.origin!==origin){json(403,{error:'Local browser disabled. Start the server with --browser.'});return true;}
  if(path==='/__gamma/browser/session'&&req.method==='GET'){json(200,{token,enabled:true,transport:'webrtc',...dimensions});return true;}
  if(req.headers['x-gamma-token']!==token){json(403,{error:'Invalid session'});return true;}
  if(busy){json(409,{error:'Browser busy'});return true;}
  ...
  let body='';try{for await(const chunk of req){body+=chunk;if(Buffer.byteLength(body)>65536){json(413,{error:'Request too large'});return true;}}}catch{return true;}
  let data;try{data=JSON.parse(body);}catch{json(400,{error:'Invalid JSON'});return true;}
  if(!data||typeof data!=='object'){json(400,{error:'Invalid command'});return true;}
  if(busy){json(409,{error:'Browser busy'});return true;}busy=true;
  try{
   if(path==='/__gamma/browser/close'){await close();json(200,{closed:true});return true;}
   if(path==='/__gamma/browser/stream'){
    if(data.type!=='offer'||typeof data.sdp!=='string'||data.sdp.length>60000)throw Error('Invalid WebRTC offer');
    json(200,{...await answer(origin,{type:data.type,sdp:data.sdp}),...dimensions});return true;
   }
   const p=await getPage();
   if(data.action==='navigate'){const url=new URL(data.url);if(!['http:','https:'].includes(url.protocol)||url.username||url.password)throw new Error('Use an HTTP or HTTPS URL without credentials.');await p.goto(url.href,{waitUntil:'domcontentloaded',timeout:20000});}
   ...
   else if(data.action==='click'){const {width,height}=p.viewport();if(!Number.isFinite(data.x)||!Number.isFinite(data.y)||data.x<0||data.y<0||data.x>width||data.y>height)throw new Error('Invalid coordinates');await p.mouse.click(data.x,data.y);}
   else if(data.action==='wheel')await p.mouse.wheel({deltaY:Math.max(-2000,Math.min(2000,Number(data.y)||0)),deltaX:0});
   else if(data.action==='text'){if(typeof data.text!=='string'||data.text.length>4000)throw new Error('Invalid text');await p.keyboard.type(data.text);}
   else if(data.action==='key'){if(!['Enter','Tab','Backspace','Delete','Escape','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Home','End','PageUp','PageDown','Space'].includes(data.key))throw new Error('Unsupported key');await p.keyboard.press(data.key);}
