...
const macDownloadUrl = `${repositoryUrl}/releases/latest/download/Gamma.Presenter-2.0.0-arm64-mac.zip`;
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{min-width:320px;margin:0;background:#f3f0e8;color:#111318;line-height:1.5}
    ...
    .product-shot{position:relative;margin:0;border-top:1px solid #111318}.product-shot img{display:block;width:100%;height:auto;border-right:1px solid #111318;border-bottom:1px solid #111318;border-left:1px solid #111318}.product-shot figcaption{display:grid;grid-template-columns:auto minmax(0,1fr);gap:18px;padding:14px 0 0;color:#62646b;font-size:.78rem;line-height:1.45}.product-shot figcaption strong{color:#111318;font-weight:760}.hero-rule{height:2px;margin:clamp(54px,8vw,110px) auto 0;background:#1748d5}
...
<body>
  ...
  <main>
    <section class="hero shell" aria-labelledby="hero-title"><div class="hero-copy"><h1 id="hero-title">Presentations with a live operating system.</h1><div class="hero-aside"><p>Gamma Presenter is the native macOS workspace for decks that carry more than text: real visuals, a real stage, a real clock, and a local AI operator that asks before it acts.</p><div class="actions"><a class="action" href="${macDownloadUrl}">Download Gamma Presenter</a><a class="action secondary" href="${releaseUrl}">Release notes</a></div><ul class="release-facts" aria-label="Current release"><li>v2.0.0</li><li>Apple silicon</li><li>177 MB ZIP</li><li>Unsigned</li></ul></div></div><figure class="product-shot"><img src="./assets/gamma-presenter-control-room.png" alt="The Gamma Presenter Author workspace showing slide source, a live Gamma preview, presentation countdown controls, action approval, and a local AI co-pilot" fetchpriority="high"><figcaption><strong>The actual Author workspace.</strong><span>Source, live Gamma render, timing, approval, and co-pilot controls share one local surface.</span></figcaption></figure></section>
    ...
    <section class="decks shell" id="decks" aria-labelledby="decks-title"><div class="deck-intro"><h2 id="decks-title">The runtime is visible in the decks.</h2><p>These are real presentations generated by the Gamma engine. Open them and inspect the output instead of taking a marketing claim on trust.</p></div>${demos || '<p>No featured presentations have been deployed yet.</p>'}</section>
    <section class="install shell" id="download" aria-labelledby="download-title"><div class="install-copy"><h2 id="download-title">Install the app. Keep the runtime.</h2><p>Gamma Presenter is available now for Apple-silicon Macs. The release contains the Gamma Presenter app in a ZIP and is explicitly unsigned while Apple Developer signing credentials are not configured.</p></div><div class="install-meta"><a class="action" href="${macDownloadUrl}">Download for Apple silicon</a><p class="download-note">Direct GitHub Release download · v2.0.0 · 177 MB ZIP · <a href="${releaseUrl}">checksums and release notes</a></p><pre class="source-command"><code>git clone ${repositoryUrl}.git
cd gamma-slides
bun install
