...
import {studioBrowserCSS,studioBrowserJS} from '../src/engine/components/studio-browser.js';
...
test('Stage browser has an English, persistent movable and resizable operator frame',()=>{const css=studioBrowserCSS(),js=studioBrowserJS();assert.match(css,/\.gamma-browser-drag/);assert.match(css,/\.gamma-browser-resize/);assert.match(css,/min-width:360px/);assert.match(js,/gamma-browser-frame-v1/);assert.match(js,/Move browser window/);assert.match(js,/Stream interrupted\. Close and reopen the browser to reconnect\./);assert.match(js,/studio-browser-resize/);assert.doesNotMatch(js,/Flux interrompu|Fermez puis rouvrez/);});
