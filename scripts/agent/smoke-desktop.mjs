...
const processLog = [];
const child = spawn(executable, [`--remote-debugging-port=${port}`, `--user-data-dir=${profile}`, '--enable-logging=stderr'], { stdio: ['ignore', 'pipe', 'pipe'] });
...
try {
  ...
  console.log(JSON.stringify({ app: basename(appBundle), author: true, mcp: true, stage: true, slides: state.slideCount, rendererReady: true }));
} finally {
