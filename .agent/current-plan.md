...
- [x] DONE — Pushed scoped implementation `e9e3a3a`; GitHub Pages run `34705679106` succeeded at 16:35:47 UTC on 2026-09-12. The actual public-URL browser run passes 7/7 (1.2 minutes): all six gallery features at four viewport sizes, lifecycle/accessibility checks, and all 23 story scenes on desktop and mobile. Deployed landing, mobile story, Archify, YouTube and mobile AI captures were inspected. All three story routes/posters, favicon and published v2.0.3 ZIP return HTTP 200. Evidence: `output/deployed-pages-validation/`, `output/deployed-gallery-20260912/`, `output/deployed-stories-20260912/`. Desktop release remains independent.
...
- [x] DONE — Publish the live gallery independently of desktop packaging. Main `0376548` deployed successfully in Pages run `34694839859` at 12:52 UTC. The actual public URL passed desktop/mobile gallery checks 2/2 in 8.3 seconds; remote YouTube and Archify captures were visually inspected. Favicon/GIF/download targets return HTTP 200. Pages resolves the latest actually published ZIP (v2.0.3), with explicit README/download copy distinguishing it from newer source. The release metadata regression passes 47/47; the measured 16:9 YouTube preview is no longer clipped.
- [x] DONE — Harden 3D renderer disposal: detach the old context-loss handler before releasing its context, dispose Three before waking Pixi, and include lifecycle helpers in generated JavaScript. Four deterministic contracts and all 14 affected browser scenarios pass. The final serial dedicated-Chromium unit/integration run passes 150/150, zero skips. Pushed as `3f29d3d`; Pages run `34695163187` succeeded at 13:00 UTC. The final remote rerun passed 2/2 in 8.6 seconds, with desktop YouTube and mobile Archify captures inspected in `output/deployed-gallery-final/`.
- [ ] IN PROGRESS — Desktop release v2.0.4 failed CI and is not published; do not overwrite its tag. Chromium is pinned, FFmpeg installed, Linux split into three shards on Ubuntu 22.04 with sandbox enabled. Browser/server teardown has two deterministic contracts and both local capture scenarios pass. Remote run `34694839778` now passes the macOS broadcast scenario, but isolated browser streaming measures 19.07 fps against the unchanged >24 fps gate. This is a distinct performance failure, not addressed by the Three patch. A successor package must pass the complete matrix before publication; this no longer blocks Pages.
...
Current local evidence: 150 unit/integration tests passed with zero skips, including isolated Chromium, four Stage concurrency tests, browser teardown and Three lifecycle regressions. The earlier full 109-scenario browser run passed with zero skips and a 60-second recording soak; the additional Writer regression brings the matrix to 110, with later targeted 15/15, capture 2/2 and Three 14/14 passes. The deployed gallery passes 2/2 on its actual public URL. The rebuilt local 2.0.4 package passed native smoke and archive integrity, but this is not a published release or a green remote CI result. Current CI failures remain explicit above. Push is authorized. Pages is delivered; desktop validation remains independent. Old DONE/BLOCKED entries below are historical, not current release sign-off.
