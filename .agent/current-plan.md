...
- [ ] IN PROGRESS — Commit and publish the validated v2.0.4 tag, verify release assets/checks, then push main and verify the deployed Pages gallery.
...
Current evidence: 144 unit/integration tests passed with zero skips, including the isolated Chromium integration and four Stage revision-concurrency tests. The full 109-scenario browser run passed with zero skips, including a 60-second recording soak; the additional Writer regression brings the matrix to 110, with final targeted desktop checks 6/6 and mobile 3D check 1/1 passing. The expired GPU texture copy into audience output is fixed and covered by a pixel regression. Writer height and mobile axis/tick overlaps are fixed, tested and visually inspected. Fresh English Writer and architecture captures replace the public legacy images. The rebuilt packaged 2.0.4 smoke passes Writer, live Archify, stable template controls, themes, MCP approval and stop → edit → Present, with private cues visible in Speaker and absent from public Stage. Preparing v2.0.4 with package/tag agreement and reusable browser quality gates before release publication. Push is explicitly authorized; publish and verify release assets before moving main/the landing to their versioned URLs. Old DONE/BLOCKED entries below are historical, not current release sign-off.
