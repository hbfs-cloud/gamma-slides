...
async function split(page){await page.locator('section.present [data-cinema-action=split]').click();await page.waitForFunction(()=>document.querySelector('section.present .cinema-stage').dataset.cinemaMoving!=='true');}
...
test('cinematic story, exact data, finite GPU work and restored presentation',async({page})=>{
 ...
 const gpu=await page.locator('section.present .cinema-canvas canvas').evaluate(c=>({pixels:c.width*c.height,error:c.getContext('webgl2').getError()}));expect(gpu.pixels).toBeLessThanOrEqual(2005000);expect(gpu.error).toBe(0);
 await page.locator('section.present [data-cinema-action=values]').click();await expect(page.getByRole('dialog')).toBeVisible();await expect(page.getByRole('row').filter({hasText:'Platforms'})).toContainText('$1.4M');await page.keyboard.press('Escape');await expect(page.getByRole('dialog')).toBeHidden();
...
test('cinematic phone composition and reduced motion',async({page})=>{
 ...
 for(const b of await page.locator('section.present .cinema-actions button').all())expect((await b.boundingBox()).height).toBeGreaterThanOrEqual(44);
 await page.locator('section.present [data-cinema-action=values]').click();await expect(page.getByRole('dialog')).toBeVisible();
});
...
test('cinematic themes, interrupted navigation, and print lifecycle',async({page})=>{
 ...
 await page.locator('section.present [data-cinema-action=split]').click();await page.evaluate(()=>Reveal.slide(2));await page.evaluate(()=>Reveal.slide(1));await expect(page.locator('section.present .cinema-stage')).toHaveAttribute('data-cinema-moving','false');await expect(page.locator('section.present .cinema-stage')).toHaveAttribute('data-cinema-mode','split');
