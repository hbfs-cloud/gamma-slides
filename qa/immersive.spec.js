...
const out = resolve('output/immersive-qa');
mkdirSync(out, { recursive: true });
const file = resolve('output/immersive-data.html');
...
test('GPU geometry, exact values, camera, selection, idle budget, and navigation', async ({ page }) => {
  ...
  await current(page).locator('.spatial-selection').selectOption('6');
  ...
  await current(page).getByRole('button', {name:'Reset view'}).click();
  ...
  await current(page).getByRole('button',{name:'Profile view',exact:true}).click();
  ...
  await current(page).getByRole('button',{name:'Reset view',exact:true}).click();
  ...
  await current(page).getByRole('button', {name:'2D view',exact:true}).click();
  ...
  await page.locator('[data-theme-open]').click();
  ...
  await expect(current(page).locator('.spatial-flat svg')).toBeVisible();
  await current(page).getByRole('button', {name:'Values',exact:true}).click();
  ...
  await current(page).getByRole('button', {name:'3D view',exact:true}).click();
  await next(page, 1);
  await expect(current(page)).toHaveAttribute('data-immersive-view', 'spatial');
  await current(page).locator('.spatial-selection').selectOption('4');
...
test('reduced motion, unavailable GPU, and context loss retain the data', async ({ browser }) => {
  ...
  await expect(current(fallback).getByRole('button',{name:'3D view',exact:true})).toBeDisabled();
...
test('export and ordinary printing produce SVG evidence without GPU allocation', async ({ page }) => {
  ...
  expect(await page.locator('canvas').count()).toBe(0);
  expect(await page.locator('.spatial-flat svg').count()).toBe(2);
...
test('phone and tablet retain reachable controls, values, and navigation', async ({page})=>{
  for(const size of [{width:390,height:844},{width:768,height:1024}]) {
    ...
    const bounds=await current(page).getByRole('button',{name:'3D view',exact:true}).boundingBox();
